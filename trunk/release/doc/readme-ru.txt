...
-----------------------------------------------------------
  ...
  Этот мод включает в себя следующие возможности:
    ...
    * Иконки игрока, клана
    * Разные наборы иконок техники для ушей, экрана загрузки и т.д.
    * Статистика игроков (только с пакетом xvm-stat)
...
-----------------------------------------------------------
  ...
  3. Если необходимо использовать в XVM вместо русского или английского другой язык ,
    то создайте файл lang.xc в папке \res_mods\xvm, содержащий в себе
    lang=xx
    Где xx - двухбуквенный код языка, который вы хотите использовать.
    Также, проверьте наличие файла xx.xc в папке \res_mods\xvm\l10n
...
-----------------------------------------------------------
  ...
  Используемые макросы:
    ...
    Макроподстановки статистики (необходимо включить rating/showPlayersStatistics):
      ...
      {{c:twr}}       - цвет в зависимости от TWR (T-Calc)
  ...
  Будет работать только при запущенном xvm-stat!

  Часы в бою и на экране загрузки.
  Формат значений: Y:год, M:месяц, D:день, H:часы, N:минуты, S:секунды. "" - убрать часы.
  Например:
  "clockFormat": "H:N"          => 01:23
  "clockFormat": "Y.M.D H:N:S"  => 2013.05.20 01:23:45
  Так же в часах в бою допустимо использовать HTML.
