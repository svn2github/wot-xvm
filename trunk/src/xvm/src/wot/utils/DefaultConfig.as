...
class wot.utils.DefaultConfig
{
  public static function get config(): Object
  {
    return
    {
      ...
      battle: {
        /* false - отключить зеркалирование иконок танков (полезно дл€ альтернативных иконок) */
        ...
        /* 0..170 - задает ширину пол€ имени игрока в "ушах" в широком режиме. ѕо умолчанию: 170 */
        playersPanelLargeWidth: 170,
        /* true - включить отображение сетки координат (может быть полезным дл€ настройки) */
        drawGrid: false
      },
      rating: {
        /* true - включить отображение статистики. √лобальный переключатель - без него модуль статистики будет везде отключен. */
        showPlayersStatistics: false,
        /* true - включить загрузку данных по противникам в "тумане войны" */
        loadEnemyStatsInFogOfWar: false,
        battleLoading: {
          /* false - отключить отображение статистики в окне загрузки бо€ */
          show: true,
          /* формат отображени€ (допускаютс€ макроподстановки) */
        ...
        playersPanel: {
          ...
          /* цвет в средних ушах. „тобы оставить стандартный, необходимо удалить значение */
          middleColor: "{{c:eff}}"
        },
        statisticForm: {
          /* false - отключить отображение статистики в окне статистики бо€ (по клавише Tab) */
          show: true,
          /* формат отображени€ */
          format: "<font color='{{c:kb}}'>{{kb}}</font> <font color='{{c:eff}}'>{{eff}}</font> <font color='{{c:rating}}'>{{rating}}</font>"
        }
