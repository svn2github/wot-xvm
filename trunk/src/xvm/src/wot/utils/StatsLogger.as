...
class wot.utils.StatsLogger
{
	...
	public function StatsLogger() 
	{
	}
	
	private static function mapData(data: Object, fieldName: String) {
		var out: Object = { }, mapping: Object = null;

		if (fieldName == "results") mapping = resultsMapping;
		else if (fieldName == "common") mapping = resultsCommonMapping;
		else if (fieldName == "player") mapping = resultsPlayerMapping;
		else if (fieldName == "stats") mapping = statPlayerMapping;
	...
	public static function saveStatistics(key: String, data: Object) {
		...
		var d: Date = new Date();
		Logger.send(d.getTime() + ":" + key + ":" + com.xvm.JSON.stringify(out, "", true), Defines.COMMAND_LOGSTAT);
	}
